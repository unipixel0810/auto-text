...
6. 확실하지 않으면 자막을 만들지 않는 것이 낫습니다. 적게 만드는 것이 지어내는 것보다 100배 낫습니다.
...
[★ 품질 기준 — 가장 중요 ★]
- ⛔ 절대로 없는 내용을 지어내지 마세요! 이것이 가장 중요한 규칙입니다.
- 대본에 이상한 말(바위, 파괴, 전쟁 등 맥락에 안 맞는 단어)이 있으면 STT 오류입니다. 무시하세요!
- 오디오에서 실제로 들리지 않는 내용은 자막으로 만들면 안 됩니다.
- 확신이 없으면 자막을 아예 만들지 마세요. 빈 구간이 낫습니다.
- 맥락이 불분명하면 상황자막(분위기/효과음 묘사)으로 대체하세요
- 자막 수가 적어도 괜찮습니다. 정확성 > 양

[일반 규칙]
- 각 자막 최소 3초 이상, 최대 6초 노출
- 자막끼리 시간 겹침 금지 (최소 2초 간격)
- 대본 텍스트 반복 금지
...
export async function POST(req: Request) {
    try {
        ...
        if (mode === 'creative' && transcriptData) {
            ...
            const targetCount = Math.max(5, Math.round(duration / 12));
