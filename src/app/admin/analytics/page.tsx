...
const HeatmapTab = memo(function HeatmapTab({
  ...
  if (!selectedPage) {
    return <EmptyState icon="heat_map" message="히트맵을 보려면 위에서 페이지를 선택해주세요." />;
  }
      ...
      <div className="flex items-center justify-between bg-[#12121a] border border-[#1e1e2e] rounded-xl p-4">
        <div className="flex items-center gap-4">
          ...
          <input
            ...
            className="w-[180px] h-2 bg-[#222] rounded-lg appearance-none cursor-pointer accent-[#00D4D4]"
          />
          <span className="text-[11px] font-mono text-[#00D4D4] w-8">{heatmapOpacity}%</span>
        </div>
        <div className="text-xs text-gray-500">
          클릭: <span className="text-[#00D4D4] font-bold">{clickEvents.length}</span>개
        </div>
      </div>

      <div className="relative bg-[#111] border border-[#1e1e2e] rounded-xl overflow-hidden" style={{ aspectRatio: `${PREVIEW_WIDTH}/${PREVIEW_HEIGHT}` }}>
        {iframeUrl && (
          <iframe
        ...
        <canvas
          ...
          style={{ opacity: heatmapOpacity / 100, mixBlendMode: 'screen', pointerEvents: 'none' }}
        />
...
export default function AnalyticsDashboard() {
  ...
  useEffect(() => {
    ...
    const canvas = canvasRef.current;
    if (!canvas) return;
    const filtered = clickEvents.filter(c => c.x_pos != null && c.y_pos != null);
    ...
    const rafId = requestAnimationFrame(() => {
      ...
      const maxVW = Math.max(...filtered.map(c => c.viewport_width || 1920), 1920);
      const maxVH = Math.max(...filtered.map(c => c.viewport_height || 1080), 1080);
      const grid = new Float32Array(w * h);
      let maxVal = 0;
      filtered.forEach(c => {
        const ix = Math.round(((c.x_pos || 0) / maxVW) * w);
        const iy = Math.round(((c.y_pos || 0) / maxVH) * h);
